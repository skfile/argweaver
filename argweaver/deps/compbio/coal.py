...
def prob_coal_counts(a, b, t, n):
    ...
    C = stats.prod((b+y)*(a-y)/(a+y) for y in range(b))
...
def prob_coal_counts_slow(a, b, t, n):
    ...
    for k in range(b, a+1):
        i = exp(-k*(k-1)*t/2.0/n) * \
            ...
            stats.prod((b+y)*(a-y)/(a+y) for y in range(k))
        s += i
    return s
...
def prob_coal_cond_counts(x, a, b, t, n):
    ...
    """
    lama = -a*(a-1)/2.0/n
    C = stats.prod((b+y)*(a-1-y)/(a-1+y) for y in range(b))
...
def sample_coal_cond_counts(a, b, t, n):
    ...
    # compute constants
    lama = -a*(a-1)/2.0/n
    C0 = stats.prod((b+y)*(a-1-y)/(a-1+y) for y in range(b))
    c = -b*(b-1)/2.0/n
...
def cdf_bounded_coal(t, k, n, T):
    ...
    lam_i = (i+1)*i/2.0 / n
    C = [mrca_const(j, 1, i-1) for j in range(1, i)]
...
def sample_bounded_coal_reject(k, n, T):
    ...
    i = k - 1
    consts = [mrca_const(j, 1, i-1) for j in range(1, i)]
    x = sum(consts)
...
def prob_multicoal_recon_topology(tree, recon, stree, n,
    ...
    for node, cnt in top_stats[1].items():
        lnp -= log(cnt)
...
def num_labeled_histories(nleaves, nroots):
    n = 1.0
    for i in range(nroots + 1, nleaves + 1):
        n *= i * (i - 1) / 2.0
    return n
...
def init_popsizes(stree, n):
    ...
    if isinstance(n, (int, float)):
        return dict.fromkeys(list(stree.nodes.keys()), n)
...
def sample_multicoal_tree(stree, n, leaf_counts=None,
    ...
    for subtree, lineages in list(subtrees.values()):
    ...
    for snode in subtrees:
        if snode not in sleaves:
            ...
            for leaf, lineage in zip(leaves, lineages2):
...
def join_subtrees(subtrees, recon, caps, sroot):
    ...
    # add all nodes to total tree
    for snode, (subtree, tops) in subtrees.items():
        tree.merge_names(subtree)
...
def make_tree_from_times(times, k=None, t=None, leaves=None, capped=False):
    ...
    if leaves is None:
        children = set(treelib.TreeNode(tree.new_name()) for i in range(k))
    else:
...
def get_topology_stats_old(tree, recon, stree, rev_recon=None):
    ...
    for snode, nodes in rev_recon.items():
