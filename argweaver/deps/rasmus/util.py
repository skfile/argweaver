...
def mapapply(funcs, lst):
    ...
    """
    return [func(item) for func, item in zip(funcs, lst)]
...
def make_matrix(nrows, ncols, val=0):
    ...
    """
    return [[val for i in range(ncols)]
...
def transpose(mat):
    ...
    """
    assert equal(* list(map(len, mat))), "rows are not equal length"
...
def find(func, *lsts):
    ...
    if len(lsts) == 1:
        ...
        assert equal(* list(map(len, lsts))), "lists are not same length"
...
def open_stream(filename, mode="r", ignore_close=True):
    ...
    # if filename is a string then open it
        ...
        if filename.startswith("http://"):
            ...
            stream = urllib.request.urlopen(filename)
...
class DelimReader:
    ...
    def __next__(self):
        ...
        row = line.rstrip("\n").split(self.delim)
        if self.types:
            return [func(x) for func, x in zip(self.types, row)]
        elif self.parse:
